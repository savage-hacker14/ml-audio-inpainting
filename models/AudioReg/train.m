...
addpath(genpath("ltfat-2.6.0-win64"))
...
for p = [256, 512, 1024, 2048, 3072]
    ...
    for method = ["arburg", "lpc"]
        ...
        gaplengths = 80; % this corresponds to what is saved in gaps_table
        ...
        rows = height(gaps_table_librispeech) * length(gaplengths);
        ...
        % start processing
        row = 0;
        for i = 1:height(gaps_table_librispeech)
        
            %% load signal
            fs = gaps_table_librispeech.fs(i);
            signal = gaps_table_librispeech.clean{i};
            ...
            for j = 1:length(gaplengths)
                ...
                signame = string(gaps_table_librispeech.Properties.RowNames{i});
                ...
                %% load mask
                mask = gaps_table_librispeech.("mask" + num2str(gaplengths(j))){i};
                gapped = signal;
                gapped(~mask) = NaN; % needed for arinpaint and janssen_inp
        
                %% write meta data to sigtable
                for m = 1:length(methods)
                    sigtables.(methods(m)).signal(j) = gaps_table_librispeech.Properties.RowNames{i};
                    sigtables.(methods(m)).fs(j) = gaps_table_librispeech.fs(i);
                    sigtables.(methods(m)).gap(j) = gaplengths(j);
                    sigtables.(methods(m)).clean{j} = gaps_table_librispeech.clean{i};
                    sigtables.(methods(m)).mask{j} = gaps_table_librispeech.("mask" + num2str(gaplengths(j))){i};
                ...
                for m = 1:length(methods)
                    ...
                    for it = 1:iterations
                        ...
                        PEMOQ(it) = audioqual(signal, solution, fs);
                        ...
                        PEAQ(it) = PQevalAudio(filename + "_signal.wav", ...
                            filename + "_solution.wav", 0, length(solution_48));
